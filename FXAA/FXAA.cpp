...
ID3D11VertexShader*         g_pSMVertexShader = NULL;     // Vertex shader for shadowmap rendering
ID3D11VertexShader*         g_pVertexShaderFXAA = NULL;   
ID3D11PixelShader*          g_pPixelShader[PATTERNS];     // Pixel shader for main scene rendering
ID3D11PixelShader*          g_pPixelShaderGather4[PATTERNS]; // Pixel shader for main scene rendering
ID3D11PixelShader*          g_pPixelShaderFXAA[6];  
...
ID3D11SamplerState*         g_pSamAni = NULL;        // Anisotropic sampler for FXAA
...
//Others
bool                        g_useGather4 = false;
int                         g_activePattern = 0;
...
#define IDC_FXAA_TEXT           4
#define IDC_FXAA_PRESET         5
...
void InitApp()
{
    ...
    iY += 40;
    g_HUD.AddStatic( IDC_FXAA_TEXT, L"FXAA Preset:", 0, iY += iYo, 170, 22 );
    g_HUD.AddComboBox( IDC_FXAA_PRESET, 0, iY += iYo, 170, 22, 'S', true ,&pComboBox );
    for (char i = 0; i < 7; i++)
    {
        pComboBox->AddItem(szSizes[i], (void*)i);
    }

	pComboBox->SetSelectedByIndex(4);
                                     ...
                                     void* pUserContext )
{
    ...
    V_RETURN( pd3dDevice->CreateVertexShader( pBlob->GetBufferPointer(), pBlob->GetBufferSize(), NULL, &g_pSMVertexShader ) );
    SAFE_RELEASE( pBlob );
    ...
    static D3D10_SHADER_MACRO Pattern_Macros[PATTERNS] = {
        {"PATTERN","44"},
        {"PATTERN","64"},
        {"PATTERN","38"},
        {"PATTERN","84"},
    ...
    static D3D10_SHADER_MACRO Gather_Macros[2] = {
        {"NOUSEGATHER4",""},
        {"USEGATHER4",""}
    };
    static D3D10_SHADER_MACRO NULLMACRO = {NULL, NULL};

    std::vector<D3D10_SHADER_MACRO> Macros;

    for (unsigned short pat = 0; pat < PATTERNS; pat++)
    {
        SAFE_RELEASE(g_pPixelShader[pat]);
        SAFE_RELEASE(g_pPixelShaderGather4[pat]);

        Macros.clear();
        Macros.push_back(Pattern_Macros[pat]);
        Macros.push_back(Gather_Macros[0]);
        Macros.push_back(NULLMACRO);

        if (pd3dDevice->GetFeatureLevel() < D3D_FEATURE_LEVEL_11_0)
            V_RETURN( D3DX11CompileFromFile( str,
                                        (const D3D10_SHADER_MACRO*)&*Macros.begin(),
                                        NULL,
                                        "RenderScenePS",
                                        "ps_4_0",
                                        dwShaderFlags,
                                        0, NULL, &pBlob, NULL, NULL  ))
        else
            V_RETURN( D3DX11CompileFromFile( str,
                                        (const D3D10_SHADER_MACRO*)&*Macros.begin(),
                                        NULL,
        ...
        V_RETURN( pd3dDevice->CreatePixelShader( pBlob->GetBufferPointer(), pBlob->GetBufferSize(), NULL, &(g_pPixelShader[pat]) ) );
        SAFE_RELEASE( pBlob );

        if (pd3dDevice->GetFeatureLevel() >= D3D_FEATURE_LEVEL_11_0)
        {   // Create Gather4 pixels shaders only for ps_5_0 or higher
            Macros.clear();
            Macros.push_back(Pattern_Macros[pat]);
            Macros.push_back(Gather_Macros[1]);
            Macros.push_back(NULLMACRO);
            ...
            V_RETURN( D3DX11CompileFromFile( str,
                                             ...
                                             dwShaderFlags,
                                             0, NULL, &pBlob, NULL, NULL ));
            V_RETURN( pd3dDevice->CreatePixelShader( pBlob->GetBufferPointer(), pBlob->GetBufferSize(), NULL, &(g_pPixelShaderGather4[pat]) ) );
            SAFE_RELEASE( pBlob );
        }
    }

    static D3D10_SHADER_MACRO Preset_Macros[6] = {
        {"FXAA_PRESET","0"},
        {"FXAA_PRESET","1"},
        {"FXAA_PRESET","2"},
        {"FXAA_PRESET","3"},
    ...
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"FXAA.hlsl" ) );
    for (unsigned short pat = 0; pat < 6; pat++)
    {
        SAFE_RELEASE(g_pPixelShaderFXAA[pat]);

        Macros.clear();
        Macros.push_back(Preset_Macros[pat]);
        ...
        V_RETURN( pd3dDevice->CreatePixelShader( pBlob->GetBufferPointer(), pBlob->GetBufferSize(), NULL, &(g_pPixelShaderFXAA[pat]) ) );
        SAFE_RELEASE( pBlob );
    }
    ...
    samDesc.MaxLOD = 0.0f;
	samDesc.MinLOD = 0.0f;
    V_RETURN( pd3dDevice->CreateSamplerState( &samDesc, &g_pSamAni ) );
                                         ...
                                         const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    if (pd3dDevice->GetFeatureLevel() < D3D_FEATURE_LEVEL_11_0)
    {   // only disable gather4 if feature level is below 11.0 (and leave default value otherwise)
        g_useGather4 = false;
    }
...
void RenderShadowMap( ID3D11Device* pd3dDevice, ID3D11DeviceContext* pd3dImmediateContext )
{
    ...
    // Set the shaders
    pd3dImmediateContext->VSSetShader( g_pSMVertexShader, NULL, 0 );
                                 ...
                                 float fElapsedTime, void* pUserContext )
{
    ...
    if (g_useGather4)
    {
        pd3dImmediateContext->PSSetShader( g_pPixelShaderGather4[g_activePattern], NULL, 0 );
    }
    else
    {
        pd3dImmediateContext->PSSetShader( g_pPixelShader[g_activePattern], NULL, 0 );
    }
    
    ID3D11SamplerState * ppSamplerStates[4] = { g_pSamPointMirror, g_pSamLinearWrap, g_pSamPointCmpClamp, g_pSamAni };
	...
	if(g_fxaaPreset)
	{
		...
		pd3dImmediateContext->PSSetShader( g_pPixelShaderFXAA[g_fxaaPreset-1], NULL, 0 );
...
void CALLBACK OnD3D11DestroyDevice( void* pUserContext )
{
    ...
    SAFE_RELEASE( g_pSMVertexShader );
    SAFE_RELEASE( g_pVertexShader );
    SAFE_RELEASE( g_pVertexShaderFXAA );
    for (int i = 0; i < PATTERNS; i++)
    {
        SAFE_RELEASE( g_pPixelShader[i] );
        SAFE_RELEASE( g_pPixelShaderGather4[i] );
    }
    for (int i = 0; i < 6; i++)
    {
        SAFE_RELEASE( g_pPixelShaderFXAA[i] );
    }
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_CHANGEDEVICE:
            ...
            break;
        case IDC_FXAA_PRESET:
            {
                CDXUTComboBox* pComboBox = (CDXUTComboBox*) pControl;
                g_fxaaPreset =  pComboBox->GetSelectedIndex();
            }
    }
}
