...
HRESULT Grid::Initialize( int gridWidth, int gridHeight, int gridDepth,void* shaderBytecode, SIZE_T shaderBytecodeLength)
{
    ...
    ComputeRowsColsForFlat3DTexture(m_dim[2], &m_cols, &m_rows);

    V_RETURN(CreateVertexBuffers(shaderBytecode, shaderBytecodeLength));
...
HRESULT Grid::CreateVertexBuffers( void* shaderBytecode, SIZE_T shaderBytecodeLength )
{
    ...
    int index(0);
    VS_INPUT_GRID_STRUCT *renderQuad(NULL);
    VS_INPUT_GRID_STRUCT *slices(NULL);
    VS_INPUT_GRID_STRUCT *boundarySlices(NULL);
    VS_INPUT_GRID_STRUCT *boundaryLines(NULL);
    ...
    V_RETURN(CreateVertexBuffer(sizeof(VS_INPUT_GRID_STRUCT)*m_numVerticesRenderQuad,
    ...
    V_RETURN(CreateVertexBuffer(sizeof(VS_INPUT_GRID_STRUCT)*m_numVerticesSlices,
    ...
    V_RETURN(CreateVertexBuffer(sizeof(VS_INPUT_GRID_STRUCT)*m_numVerticesBoundarySlices,
    ...
    V_RETURN(CreateVertexBuffer(sizeof(VS_INPUT_GRID_STRUCT)*m_numVerticesBoundaryLines,
...
void Grid::InitScreenSlice(VS_INPUT_GRID_STRUCT** vertices, int z, int& index )
{
    VS_INPUT_GRID_STRUCT tempVertex1;
    VS_INPUT_GRID_STRUCT tempVertex2;
    VS_INPUT_GRID_STRUCT tempVertex3;
    VS_INPUT_GRID_STRUCT tempVertex4;
    ...
    float Width  = float(m_cols * m_dim[0])*2.0f;
    float Height = float(m_rows * m_dim[1])*2.0f;
...
void Grid::InitSlice( int z, VS_INPUT_GRID_STRUCT** vertices, int& index )
{
    VS_INPUT_GRID_STRUCT tempVertex1;
    VS_INPUT_GRID_STRUCT tempVertex2;
    VS_INPUT_GRID_STRUCT tempVertex3;
...
{
    VS_INPUT_GRID_STRUCT tempVertex;
    int w = m_dim[0];
    int h = m_dim[1];
...
void Grid::InitBoundaryQuads( VS_INPUT_GRID_STRUCT** vertices, int& index )
{
    InitSlice( 0, vertices, index );
    InitSlice( m_dim[2]-1, vertices, index );
}

void Grid::InitBoundaryLines( VS_INPUT_GRID_STRUCT** vertices, int& index )
{
    int w = m_dim[0];
    int h = m_dim[1];
...
{
    UINT stride[1] = { sizeof(VS_INPUT_GRID_STRUCT) };
    UINT offset[1] = { 0 };
...
HRESULT Grid::CreateVertexBuffer( int ByteWidth, UINT bindFlags, ID3D11Buffer** vertexBuffer,
                                  VS_INPUT_GRID_STRUCT* vertices,int numVertices)
{
    HRESULT hr(S_OK);
